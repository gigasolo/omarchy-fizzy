...
The marketplace listing is `preview.png` at the repository root (the
shortcuts overlay). `docs/help.png` is the same image. Do not upload the
raw list or peek screenshots from testing; they showed a live board.

## Public blurb

Unread Fizzy notifications in your Omarchy bar. New for you is your inbox,
not every new card on the board. Peek at a card and its comments, copy the
link, send it to your Omarchy agent, or mark a ping read. If the Fizzy CLI
is not installed yet, the panel shows how to finish setup.
...
Bar widget (`gigasolo.fizzy`). Depends on the Fizzy CLI (`omarchy pkg aur add fizzy-cli`). The plugin never reads `FIZZY_TOKEN` or `~/.fizzy.yaml`; it only runs `fizzy` subcommands, `wl-copy`, and `omarchy-agent-prompt`. Missing CLI or unsigned-in accounts get a setup card that launches `omarchy pkg aur add fizzy-cli` and `fizzy setup` in a visible terminal. `preview.png` is the shortcuts overlay. MIT, GigaSolo LLC.
